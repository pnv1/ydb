...
static arrow20::Status SimpleNullaryExec(cp::KernelContext* ctx, const cp::ExecSpan&, cp::ExecResult* out) {
    auto scalar = arrow20::MakeScalar(TOperator:: template Call<typename cp::internal::GetViewType<TRes>::T>(ctx));
    auto result = arrow20::MakeArrayFromScalar(*scalar, 1);
    if (!result.ok()) {
