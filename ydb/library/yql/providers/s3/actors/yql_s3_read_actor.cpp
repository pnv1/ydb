...
void ThrowParquetNotOk(arrow20::Status status) {
    if (!status.ok()) {
        throw parquet20::ParquetException(status.ToString());
    }
}
...
public:
    ...
    void RunCoroBlockArrowParserOverHttp() {
        ...
        std::vector<std::unique_ptr<parquet20::arrow20::FileReader>> readers;

        parquet20::arrow20::FileReaderBuilder builder;
        builder.memory_pool(arrow20::default_memory_pool());
        parquet20::ArrowReaderProperties properties;
        ...
        if (numGroups) {
            ...
            for (ui64 i = 0; i < readerCount; i++) {
                if (!columnIndices.empty()) {
                    ...
                    // WillNeedRowGroups removed in arrow20, data loading is now synchronous in ReadRowGroups
            ...
            while (readyGroupCount < numGroups) {
                ...
                ThrowParquetNotOk(readers[readyReaderIndex]->ReadRowGroups({ hasPredicate ? static_cast<int>(matchedRowGroups[readyGroupIndex]) : static_cast<int>(readyGroupIndex) }, columnIndices, &table));
                readyGroupCount++;
        ...
        try {
            if (ReadSpec->Arrow) {
                if (ReadSpec->Compression) {
                    ...
                    } catch (const parquet20::ParquetException& ex) {
