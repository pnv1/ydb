...
using ArrowFileReaderPtr = std::unique_ptr<parquet20::arrow20::FileReader>;
...
public:
    TFileReaderWrapper(const TArrowFileDesc& desc) {
        ...
        auto fileReaderResult = parquet20::arrow20::OpenFile(ArrowFile, arrow20::default_memory_pool());
        THROW_ARROW_NOT_OK(fileReaderResult.status());
        FileReader = std::move(fileReaderResult).ValueUnsafe();
    }
