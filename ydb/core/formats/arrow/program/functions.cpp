...
struct SumImpl : public arrow20::compute::KernelState {
  ...
  arrow20::Status Consume(arrow20::compute::KernelContext*, const arrow20::compute::ExecBatch& batch) {
    if (batch[0].is_array()) {
      ...
      if (arrow20::is_boolean_type<ArrowType>::value) {
        ...
        // Simple sum implementation (SumArray not available in new Arrow)
        arrow20::NumericArray<ArrowType> array(data);
        for (int64_t i = 0; i < array.length(); ++i) {
          if (array.IsValid(i)) {
            this->Sum += static_cast<typename SumType::c_type>(array.Value(i));
          }
        }
      }
    } else {
  ...
  arrow20::Status MergeFrom(arrow20::compute::KernelContext*, arrow20::compute::KernelState&& src) {
  ...
  arrow20::Status Finalize(arrow20::compute::KernelContext*, arrow20::Datum* out) {
...
void AddScalarAggKernels(arrow20::compute::KernelInit init,
  ...
  for (const auto& ty : types) {
    ...
    auto sig = arrow20::compute::KernelSignature::Make({ty}, out_ty);
    arrow20::compute::internal::AddAggKernel(std::move(sig), init, func, arrow20::compute::SimdLevel::NONE);
  }
}
...
void AddArrayScalarAggKernels(arrow20::compute::KernelInit init,
  ...
  arrow20::compute::internal::AddBasicAggKernels(init, types, out_ty, func, simd_level);
  AddScalarAggKernels(init, types, out_ty, func);
}

// SumInit temporarily disabled - using default Arrow implementation
...
//   // SumLikeInit is not available in new Arrow version
//   return arrow20::Status::NotImplemented("Custom SumInit");
// }
    ...
    if (func == "sum") {
        ...
        for (const arrow20::compute::ScalarAggregateKernel* kernel : aggregateFunc->kernels()) {
            ...
            if (shouldReplaceKernel(*kernel)) {
                // Custom SumInit disabled - using default Arrow implementation
