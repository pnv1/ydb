...
public:
    ...
    virtual std::shared_ptr<arrow20::ChunkedArray> GetChunkedArrayTrivial() const override {
        return std::make_shared<arrow20::ChunkedArray>(Array);
    }

    const std::shared_ptr<arrow20::Array>& GetArray() const {
        return Array;
    }

    static std::shared_ptr<TTrivialArray> BuildEmpty(const std::shared_ptr<arrow20::DataType>& type);

    TTrivialArray(const std::shared_ptr<arrow20::Array>& data)
        : TBase(data->length(), EType::Array, data->type())
        , Array(data) {
    }

    static std::shared_ptr<arrow20::Array> BuildArrayFromScalar(const std::shared_ptr<arrow20::Scalar>& scalar) {
        AFL_VERIFY(scalar);
        return TStatusValidator::GetValid(arrow20::MakeArrayFromScalar(*scalar, 1));
    }

    static std::shared_ptr<arrow20::Array> BuildArrayFromOptionalScalar(
        const std::shared_ptr<arrow20::Scalar>& scalar, const std::shared_ptr<arrow20::DataType>& type);

    TTrivialArray(const std::shared_ptr<arrow20::Scalar>& scalar)
        : TBase(1, EType::Array, TValidator::CheckNotNull(scalar)->type)
        , Array(BuildArrayFromScalar(scalar)) {
    }

    template <class TArrowDataType = arrow20::StringType>
    class TPlainBuilder {
    private:
    ...
    public:
        TPlainBuilder(const ui32 reserveItems = 0, const ui32 reserveSize = 0) {
            Builder = NArrow::MakeBuilder(arrow20::TypeTraits<TArrowDataType>::type_singleton(), reserveItems, reserveSize);
        }
    ...
    static TPlainBuilder<arrow20::StringType> MakeBuilderUtf8(const ui32 reserveItems = 0, const ui32 reserveSize = 0) {
        return TPlainBuilder<arrow20::StringType>(reserveItems, reserveSize);
    }
};
...
private:
    using TBase = IChunkedArray;
    const std::shared_ptr<arrow20::ChunkedArray> Array;
...
public:
    TTrivialChunkedArray(const std::shared_ptr<arrow20::ChunkedArray>& data)
