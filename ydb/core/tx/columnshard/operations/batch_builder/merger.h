...
public:
    IMerger(const NArrow::TContainerWithIndexes<arrow20::RecordBatch>& incoming, const std::shared_ptr<ISnapshotSchema>& actualSchema)
    ...
    virtual NArrow::TContainerWithIndexes<arrow20::RecordBatch> BuildResultBatch() = 0;

    TYdbConclusionStatus Finish();

    TYdbConclusionStatus AddExistsDataOrdered(const std::shared_ptr<arrow20::Table>& data);
};
...
public:
    using TBase::TBase;
    virtual NArrow::TContainerWithIndexes<arrow20::RecordBatch> BuildResultBatch() override {
        return IncomingData;
    }
};
...
private:
    ...
    std::vector<std::shared_ptr<arrow20::BooleanArray>> HasIncomingDataFlags;
...
public:
    ...
    TUpdateMerger(const NArrow::TContainerWithIndexes<arrow20::RecordBatch>& incoming, const std::shared_ptr<ISnapshotSchema>& actualSchema,
