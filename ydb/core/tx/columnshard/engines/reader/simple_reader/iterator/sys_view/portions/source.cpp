...
std::shared_ptr<arrow20::Array> TSourceData::BuildArrayAccessor(const ui64 columnId, const ui32 recordsCount) const {
    ...
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::ColumnRawBytes::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint64());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt64Type>(*builder, i->GetMeta().GetColumnRawBytes());
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::IndexRawBytes::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint64());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt64Type>(*builder, i->GetMeta().GetIndexRawBytes());
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::ColumnBlobBytes::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint64());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt64Type>(*builder, i->GetMeta().GetColumnBlobBytes());
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::IndexBlobBytes::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint64());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt64Type>(*builder, i->GetMeta().GetIndexBlobBytes());
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::PortionId::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint64());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt64Type>(*builder, i->GetPortionId());
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::Activity::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint8());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt8Type>(*builder, i->HasRemoveSnapshot() ? 0 : 1);
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    ...
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::Stats::ColumnId) {
        ...
        return NArrow::TStatusValidator::GetValid(arrow20::MakeArrayFromScalar(arrow20::StringScalar(statInfo), recordsCount));
    }
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::Optimized::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint8());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt8Type>(*builder, i->HasRuntimeFeature(TPortionInfo::ERuntimeFeature::Optimized) ? 1 : 0);
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
    ...
    if (columnId == NKikimr::NSysView::Schema::PrimaryIndexPortionStats::CompactionLevel::ColumnId) {
        auto builder = NArrow::MakeBuilder(arrow20::uint64());
        for (auto&& i : Portions) {
            NArrow::Append<arrow20::UInt64Type>(*builder, i->GetMeta().GetCompactionLevel());
        }
        return NArrow::FinishBuilder(std::move(builder));
    }
