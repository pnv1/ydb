...
public:
    ...
    arrow20::Datum Convert(std::shared_ptr<arrow20::ArrayData> data) override final {
        if (arrow20::Type::DICTIONARY == data->type->id()) {
            ...
            if (arrow20::Type::STRING == data->type->id()) {
                auto res = arrow20::compute::Cast(arrow20::Datum(data), arrow20::compute::CastOptions::Safe(arrow20::TypeHolder(std::make_shared<arrow20::BinaryType>())));
                Y_ENSURE(res.ok());
                data = res->array();
            }
    ...
    arrow20::Datum ConvertDict(std::shared_ptr<arrow20::ArrayData> data) {
        ...
        if (arrow20::Type::STRING == data->dictionary->type->id()) {
            auto res = arrow20::compute::Cast(arrow20::Datum(data->dictionary), arrow20::compute::CastOptions::Safe(arrow20::TypeHolder(std::make_shared<arrow20::BinaryType>())));
            Y_ENSURE(res.ok());
