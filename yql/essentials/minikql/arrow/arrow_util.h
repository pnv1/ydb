...
inline void* GetPrimitiveScalarValueMutablePtr(arrow20::Scalar& scalar) {
    return const_cast<void*>(dynamic_cast<arrow20::internal::PrimitiveScalarBase&>(scalar).data());
}
...
struct TPrimitiveDataType<NYql::NDecimal::TInt128> {
    ...
    public:
        TScalarResult(std::shared_ptr<arrow20::Buffer> value)
            : arrow20::FixedSizeBinaryScalar(std::move(value), arrow20::fixed_size_binary(16))
        {
        }

        TScalarResult()
            : arrow20::FixedSizeBinaryScalar(arrow20::Buffer::FromString(""), arrow20::fixed_size_binary(16))
        {
        }
    };
};
