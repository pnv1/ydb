...
public:
    ...
    void MakeBlocks(const THolderFactory& holderFactory) {
        Values.back() = holderFactory.CreateArrowBlock(arrow20::Datum(std::make_shared<arrow20::UInt64Scalar>(OutputRows_)));
        OutputRows_ = 0;
    ...
    TBlockItem GetItem(size_t idx, size_t offset = 0) const {
        ...
        ARROW_DEBUG_CHECK_DATUM_TYPES(InputsDescr_[idx], datum.type());
        if (datum.is_scalar()) {
...
public:
    struct TBlock {
        ...
        TBlock() = default;
        TBlock(size_t size, std::vector<arrow20::Datum> columns)
            : Size(size)
    ...
    bool FetchNextBlock() {
        ...
        std::vector<arrow20::Datum> blockColumns(Readers_.size());
        for (size_t i = 0; i < Readers_.size(); i++) {
            ...
            ARROW_DEBUG_CHECK_DATUM_TYPES(InputsDescr_[i], datum.type());
            if (datum.is_scalar()) {
