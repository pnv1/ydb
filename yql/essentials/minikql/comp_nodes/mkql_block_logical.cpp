...
bool IsAllEqualsTo(const arrow20::compute::ExecValue& value_span, bool value) {
    if (value_span.is_scalar()) {
        const auto& scalar = static_cast<const arrow20::UInt8Scalar&>(*value_span.scalar);
        if (!scalar.is_valid) {
            return false;
        }
        return (scalar.value & 1u) == value;
    }
    const auto& array = value_span.array;
    if (array.GetNullCount() != 0) {
        return false;
    }
    size_t len = array.length;
    size_t popCnt = GetSparseBitmapPopCount(array.GetValues<ui8>(1), len);
...
public:
    arrow20::Status Exec(arrow20::compute::KernelContext* ctx, const arrow20::compute::ExecSpan& batch, arrow20::compute::ExecResult* res) const {
        const auto& firstDatum = batch[0];
        const auto& secondDatum = batch[1];
        if (firstDatum.is_scalar() && secondDatum.is_scalar()) {
            res->value = CalcScalarScalar(firstDatum, secondDatum);
            return arrow20::Status::OK();
        }
        if (IsAllEqualsTo(firstDatum, false)) {
            // false AND ... = false
            if (firstDatum.is_array()) {
                res->value = firstDatum.array.ToArrayData();
            } else {
                // need length
                res->value = MakeFalseArray(ctx->memory_pool(), secondDatum.array.length).array();
            }
        ...
        if (IsAllEqualsTo(secondDatum, false)) {
            ...
            if (secondDatum.is_array()) {
                res->value = secondDatum.array.ToArrayData();
            } else {
                res->value = MakeFalseArray(ctx->memory_pool(), firstDatum.array.length).array();
            }
        ...
        if (firstDatum.is_scalar()) {
            const auto& scalar = static_cast<const arrow20::UInt8Scalar&>(*firstDatum.scalar);
            ui8 value = scalar.value & 1u;
            bool valid = scalar.is_valid;
            res->value = CalcScalarArray(ctx->memory_pool(), value, valid, secondDatum.array.ToArrayData()).array();
        } else if (secondDatum.is_scalar()) {
            const auto& scalar = static_cast<const arrow20::UInt8Scalar&>(*secondDatum.scalar);
            ui8 value = scalar.value & 1u;
            bool valid = scalar.is_valid;
            res->value = CalcScalarArray(ctx->memory_pool(), value, valid, firstDatum.array.ToArrayData()).array();
        } else {
            res->value = CalcArrayArray(ctx->memory_pool(), firstDatum.array.ToArrayData(), secondDatum.array.ToArrayData()).array();
        }
    ...
    arrow20::Status Exec(arrow20::compute::KernelContext* ctx, const arrow20::compute::ExecSpan& batch, arrow20::compute::ExecResult* res) const {
        ...
        if (IsAllEqualsTo(firstDatum, true)) {
            ...
            if (firstDatum.is_array()) {
                ...
                // need length
                res->value = MakeTrueArray(ctx->memory_pool(), secondDatum.array.length).array();
            }
        ...
        if (IsAllEqualsTo(secondDatum, true)) {
            ...
            if (secondDatum.is_array()) {
                ...
                res->value = MakeTrueArray(ctx->memory_pool(), firstDatum.array.length).array();
            }
...
public:
    arrow20::Status Exec(arrow20::compute::KernelContext* ctx, const arrow20::compute::ExecSpan& batch, arrow20::compute::ExecResult* res) const {
        ...
        if (firstDatum.array.GetNullCount() == firstDatum.array.length) {
            if (firstDatum.is_array()) {
                res->value = firstDatum.array.ToArrayData();
            } else {
                res->value = MakeNullArray(ctx->memory_pool(), secondDatum.array.length);
            }

            return arrow20::Status::OK();
        }

        if (secondDatum.array.GetNullCount() == secondDatum.array.length) {
            if (secondDatum.is_array()) {
                res->value = secondDatum.array.ToArrayData();
            } else {
                res->value = MakeNullArray(ctx->memory_pool(), firstDatum.array.length);
            }
        ...
        if (firstDatum.is_scalar()) {
            const auto& scalar = static_cast<const arrow20::UInt8Scalar&>(*firstDatum.scalar);
            ui8 value = scalar.value;
            res->value = CalcScalarArray(ctx->memory_pool(), value, secondDatum.array.ToArrayData()).array();
        } else if (secondDatum.is_scalar()) {
            const auto& scalar = static_cast<const arrow20::UInt8Scalar&>(*secondDatum.scalar);
            ui8 value = scalar.value;
            res->value = CalcScalarArray(ctx->memory_pool(), value, firstDatum.array.ToArrayData()).array();
        } else {
            res->value = CalcArrayArray(ctx->memory_pool(), firstDatum.array.ToArrayData(), secondDatum.array.ToArrayData()).array();
        }
...
public:
    arrow20::Status Exec(arrow20::compute::KernelContext* ctx, const arrow20::compute::ExecSpan& batch, arrow20::compute::ExecResult* res) const {
        ...
        if (input.is_scalar()) {
            const auto& arg = static_cast<const arrow20::UInt8Scalar&>(*input.scalar);
            res->value = arg.is_valid ? MakeScalarDatum(bool(~arg.value & 1u)) : arrow20::Datum(input.scalar);
            return arrow20::Status::OK();
        }
        MKQL_ENSURE(input.is_array(), "Expected array");
        const auto& arr = input.array;
        if (arr.GetNullCount() == arr.length) {
            res->value = input.array.ToArrayData();
        } else {
            ...
            res->value = arrow20::ArrayData::Make(arr.type, arr.length, {bitmap, data});
        }
...
arrow20::Status LogicalKernelExec(arrow20::compute::KernelContext* ctx, const arrow20::compute::ExecSpan& batch, arrow20::compute::ExecResult* res) {
