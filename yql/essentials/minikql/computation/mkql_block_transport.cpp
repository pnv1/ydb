...
std::shared_ptr<arrow20::Buffer> MakeZeroBuffer(size_t byteLen) {
    ...
    if (capacity <= NullWordCount * sizeof(ui64)) {
        return std::make_shared<arrow20::Buffer>(AlignUp(reinterpret_cast<const ui8*>(nulls), ArrowMemoryAlignment), byteLen);
    }
...
public:
    ...
    virtual std::shared_ptr<arrow20::ArrayData> LoadArray(TChunkedBuffer& src, ui64 blockLen, TMaybe<size_t> offset) final {
