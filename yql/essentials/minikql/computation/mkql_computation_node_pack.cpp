                                            ...
                                            const THolderFactory& holderFactory, TPackerState& s)
{
    switch (type->GetKind()) {
        ...
        case TType::EKind::Optional: {
            ...
            if (present) {
                return UnpackFromChunkedBuffer<Fast>(optionalType->GetItemType(),
                                                     buf, topLength, holderFactory, s)
                    .Release()
                    .MakeOptional();
            } else {
                return NUdf::TUnboxedValuePod();
        ...
        case TType::EKind::List: {
            ...
            for (ui64 i = 0; i < len; ++i) {
                tmp.emplace_back(UnpackFromChunkedBuffer<Fast>(itemType,
                                                               buf, topLength, holderFactory, s));
            }
        ...
        case TType::EKind::Variant: {
            ...
            if (innerType->IsStruct()) {
                MKQL_ENSURE(variantIndex < static_cast<TStructType*>(innerType)->GetMembersCount(),
                            "Bad variant index: " << variantIndex);
                ...
                MKQL_ENSURE(variantIndex < static_cast<TTupleType*>(innerType)->GetElementsCount(),
                            "Bad variant index: " << variantIndex);
                innerType = static_cast<TTupleType*>(innerType)->GetElementType(variantIndex);
            }
            return holderFactory.CreateVariantHolder(
                UnpackFromChunkedBuffer<Fast>(innerType,
                                              buf, topLength, holderFactory, s)
                    .Release(), variantIndex);
        }
...
template <bool Fast>
NUdf::TUnboxedValue DoUnpack(const TType* type, TChunkedInputBuffer& buf, size_t totalBufSize,
                             const THolderFactory& holderFactory, TPackerState& s) {
    if constexpr (Fast) {
...
template <bool Fast>
void DoUnpackBatch(const TType* type, TChunkedInputBuffer& buf, size_t totalSize,
                   const THolderFactory& holderFactory, TPackerState& s, TUnboxedValueBatch& result) {
    ui64 len;
    ui32 topLength;
    ...
    switch (type->GetKind()) {
        ...
        case TType::EKind::List: {
            ...
            if (value.HasFastListLength()) {
                ...
                TThresher<false>::DoForEachItem(
                    value,
                    [&](const NYql::NUdf::TUnboxedValuePod& item) {
...
template <bool Fast>
void TValuePackerTransport<Fast>::UnpackBatch(TChunkedBuffer&& buf,
                                              const THolderFactory& holderFactory,
                                              TUnboxedValueBatch& result) const {
    if (IsBlock_) {
...
TValuePackerTransport<Fast>& TValuePackerTransport<Fast>::AddWideItemBlocks(const NUdf::TUnboxedValuePod* values, ui32 width) {
    ...
    for (size_t i = 0; i < width; ++i) {
        ...
        if (datum.is_array()) {
            ...
            if (!ConvertedScalars_[i]) {
                const TType* itemType = IsLegacyBlock_
                                            ? static_cast<const TStructType*>(Type_)->GetMemberType(i)
                ...
                MKQL_ENSURE(HasOffset(*datum.array(), 0),
                            "Expected zero array offset after scalar is converted to array");
    ...
    MKQL_ENSURE(savedMetadata == totalMetadataCount,
                "Serialization metadata error. Expected: " << totalMetadataCount << ", actual: " << savedMetadata);
...
void TValuePackerTransport<Fast>::BuildMeta(TPagedBuffer::TPtr& buffer, bool addItemCount) const {
    ...
    if (char* header = buffer->Header(metaSize + itemCountSize)) {
        ...
        SerializeMeta(buf, useMask, s.OptionalUsageMask, fullLen,
                      s.Properties.Test(EPackProps::SingleOptional));
        if (addItemCount) {
            if constexpr (Fast) {
        ...
        SerializeMeta(*resultBuffer, useMask, s.OptionalUsageMask,
                      fullLen, s.Properties.Test(EPackProps::SingleOptional));
        if (addItemCount) {
