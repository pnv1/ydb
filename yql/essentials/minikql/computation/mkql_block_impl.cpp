...
arrow20::TypeHolder ToValueDescr(TType* type) {
    arrow20::TypeHolder ret;
    MKQL_ENSURE(ConvertInputArrowType(type, ret), "can't get arrow type");
    return ret;
}

std::vector<arrow20::TypeHolder> ToValueDescr(const TVector<TType*>& types) {
...
std::vector<arrow20::compute::InputType> ConvertToInputTypes(const TVector<TType*>& argTypes) {
    ...
    for (auto& type : argTypes) {
        result.emplace_back(arrow20::compute::InputType(ToValueDescr(type).GetSharedPtr()));
    }
    return result;
}

arrow20::compute::OutputType ConvertToOutputType(TType* output) {
    return arrow20::compute::OutputType(ToValueDescr(output).GetSharedPtr());
}
...
NUdf::TUnboxedValuePod TBlockFuncNode::DoCalculate(TComputationContext& ctx) const {
    ...
    if (ScalarOutput_) {
        ...
        ARROW_OK(executor->Execute(arrow20::compute::ExecBatch(argDatums, argDatums.empty() ? 0 : argDatums[0].length()), listener.get()));
    ...
    while (dechunker.Next(chunk)) {
        ...
        ARROW_OK(executor->Execute(arrow20::compute::ExecBatch(chunk, chunk.empty() ? 0 : chunk[0].length()), &listener));
